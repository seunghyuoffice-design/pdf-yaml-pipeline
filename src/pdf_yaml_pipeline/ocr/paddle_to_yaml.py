...
Example:
    >>> from pdf_yaml_pipeline.ocr.paddle_to_yaml import PaddleToYAML
...
class PaddleToYAML:
    ...
    def _run_ocr_on_page(self, page_image) -> List[PaddleOCRResult]:
        ...
        # PaddleOCR 실행 (API 2.9+ predict 사용)
        raw = self._ocr.predict(img_array)

        # Generator 또는 list 처리
        ...
        for i, text in enumerate(texts):
            ...
            polygon = boxes[i] if i < len(boxes) else []
