...
def _is_optional_dependency_error(exc: Exception) -> bool:
    ...
    try:
        ...
        PanicException = ()
    if PanicException and isinstance(exc, PanicException):
        return True
    return False
...
class MultiParserSystem:
    ...
    def _get_default_parsers(self) -> List[Tuple[str, Callable]]:
        ...
        # Primary: pypdf (BSD-3-Clause)
        try:
            import pypdf
            parsers.append(('pypdf', self._pypdf_parser))
        except Exception as e:
            if _is_optional_dependency_error(e):
                logger.warning(f"pypdf not available, skipping: {e}")
            else:
                raise
        ...
        try:
            ...
            if _is_optional_dependency_error(e):
                logger.warning(f"pdfplumber not available, skipping: {e}")
            else:
                raise

        # Note: AGPL-licensed parsers are excluded (MIT/Apache-2.0/BSD only)
        # For OCR, use PaddleOCR in separate stage.
