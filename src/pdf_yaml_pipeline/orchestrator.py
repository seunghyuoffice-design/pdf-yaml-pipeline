...
from pdf_yaml_pipeline.converters.jsonl_converter import JSONLConverter, MultiFormatConverter
...
# Backward-compat alias to avoid confusion with training pipeline configs.
ConversionPipelineConfig = PipelineConfig
...
class Pipeline:
    ...
    def _classify_document(self, file_path: Path, parsed_doc: Any) -> DocumentType:
        """문서 유형 분류."""
        label = classify_document_label(file_path.name, self._extract_text(parsed_doc))
        label_map = {
            "terms_and_conditions": DocumentType.TERMS_AND_CONDITIONS,
            "business_method": DocumentType.BUSINESS_METHOD,
            "product_summary": DocumentType.PRODUCT_SUMMARY,
            "fss_release": DocumentType.FSS_PRESS_RELEASE,
            "court_precedent": DocumentType.COURT_PRECEDENT,
        }
        return label_map.get(label, DocumentType.UNKNOWN)

    def _extract_text(self, parsed_doc: Any) -> str:
        """파싱된 문서에서 텍스트 추출."""
        ...
        if hasattr(parsed_doc, "markdown"):
            return parsed_doc.markdown or ""
        if hasattr(parsed_doc, "raw_text"):
            return parsed_doc.raw_text or ""
        if hasattr(parsed_doc, "text"):
            return parsed_doc.text or ""
    ...
    def _create_examples(
        ...
        text = self._extract_text(parsed_doc)
