...
class YAMLConverter:
    ...
    def _classify_yaml_doc(self, doc: Dict[str, Any]) -> str:
        ...
        # 파일명 기반 분류
        if "약관" in source_path or "terms" in source_path:
            return "terms_and_conditions"
        elif "사업방법서" in source_path or "business" in source_path:
            return "business_method"
        elif "요약서" in source_path or "summary" in source_path:
            return "product_summary"
        elif "금감원" in source_path or "fss" in source_path:
            return "fss_release"
        elif "판례" in source_path or "precedent" in source_path:
            return "court_precedent"

        # 내용 기반 분류
        if "보험" in content and "약관" in content:
            return "terms_and_conditions"
        elif "보험료" in content and "산출" in content:
            return "business_method"
        elif "청구" in content and "절차" in content:
    ...
    def _classify_document(self, doc: ParsedDocument) -> str:
        ...
        if "약관" in doc.source_path.lower() or "terms" in doc.source_path.lower():
            return "terms_and_conditions"
        elif "사업방법서" in doc.source_path.lower() or "business" in doc.source_path.lower():
            return "business_method"
        elif "요약서" in doc.source_path.lower() or "summary" in doc.source_path.lower():
            return "product_summary"
        elif "금감원" in doc.source_path.lower() or "fss" in doc.source_path.lower():
            return "fss_release"
        elif "판례" in doc.source_path.lower() or "precedent" in doc.source_path.lower():
            return "court_precedent"
    ...
    def _to_openai(self, data: Dict) -> Dict:
        ...
        if self.include_metadata and data.get("metadata"):
            result["_meta"] = data["metadata"]
    ...
    def _to_qwen3(self, data: Dict) -> Dict:
        ...
        # Assistant 응답에 thinking 추가 (옵션)
        output = data.get("output", "")
        if self.output_format == OutputFormat.QWEN3 and len(output) > 100:
            # 긴 답변에 thinking 추가
            thinking = "사용자 질문에 답변하기 위해 관련 정보를 체계적으로 정리해보겠습니다.\n"
            thinking += "1. 질문의 핵심 파악\n"
            thinking += "2. 관련 규정과 내용 검토\n"
            thinking += "3. 논리적인 답변 구성\n\n"
