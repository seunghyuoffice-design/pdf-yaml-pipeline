...
def table_rows_to_sentences(table: Dict[str, Any], min_rel: float = 0.6) -> List[str]:
    """테이블 행을 문장으로 변환."""
    ...
    for r in range(1, rows):
        parts = [f"{headers[c]}: {grid[r][c]}" for c in range(cols) if grid[r][c] and rel[r][c] >= min_rel]
        if parts:
...
def build_rag_chunks(doc: Dict[str, Any]) -> List[Dict[str, Any]]:
    """YAML 문서에서 RAG 청크 생성.
    ...
    meta_base = {
        ...
        "format": d.get("format"),
        "role": role.value,
        "product_id": d.get("product_id") or d.get("source_path", "").split("/")[-2],
    ...
    if role == DocumentRole.CANONICAL:
        ...
        for t in doc.get("tables") or []:
            ...
            for s in table_rows_to_sentences(t):
                chunks.append(
                    {
                        "text": s,
                        "meta": {
