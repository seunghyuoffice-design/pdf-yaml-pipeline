...
from pdf_yaml_pipeline.parsers.config_schema import (  # noqa: E402
    _get_probation_timeout,
)
from pdf_yaml_pipeline.utils.timeout_calculator import calculate_timeout  # noqa: E402
...
def _parse_worker_loop(
    ...
    Multi-parser fallback: primary parser 실패 시 pypdf, pdfplumber 순차 시도
    """
    from pathlib import Path
    ...
    while True:
        ...
        try:
            # Try primary parser (UnifiedParser)
            result = parser.parse(file_path)
...
class SafeParser:
        ...
        if self._worker is not None:
            try:
                if self._worker.is_alive():
                    ...
                    if self._req_q is not None:
                        try:
                            ...
                            logger.debug(f"Failed to enqueue shutdown signal: {e}")
                ...
                try:
                    ...
                    logger.debug(f"Worker close failed: {e}")
        ...
        for q in (self._req_q, self._res_q):
            if q is not None:
                try:
                    ...
                    logger.debug(f"Queue close failed: {e}")
                try:
                    q.join_thread()
                except Exception as e:
                    logger.debug(f"Queue join_thread failed: {e}")
        self._req_q = None
        self._res_q = None
    ...
    def __del__(self):
        ...
        try:
            ...
            logger.debug(f"SafeParser __del__ cleanup skipped: {e}")
