...
from pdf_yaml_pipeline.quality.table_quality import calculate_cell_reliability
from pdf_yaml_pipeline.rag.chunk_builder import (
    build_rag_chunks as build_rag_chunks_from_dict,
    table_rows_to_sentences as dict_table_rows_to_sentences,
)
...
def table_to_dict(table: TableYAML) -> Dict[str, Any]:
    """Convert TableYAML to serializable dictionary."""
    return {
        ...
        "quality": None
        if table.quality is None
        else {
            "passed": table.quality.passed,
            "action": table.quality.action,
            "fill_rate": table.quality.fill_rate,
            "avg_reliability": table.quality.avg_reliability,
            "low_conf_ratio": table.quality.low_conf_ratio,
            "header_ok": table.quality.header_ok,
        },
        "cells": [
            {
                "row": cell.row,
...
def parsed_document_to_dict(doc: ParsedDocumentYAML) -> Dict[str, Any]:
    ...
    return {
        ...
        "tables": [table_to_dict(table) for table in doc.tables],
        "assets": {
            "images": [
                {
...
# Reliability Scoring
# =============================================================================
# calculate_cell_reliability is imported from pdf_yaml_pipeline.quality.table_quality
...
def table_rows_to_sentences(table: TableYAML, min_reliability: float = 0.60) -> List[str]:
    """Convert table rows to RAG-indexable sentences (wrapper)."""
    return dict_table_rows_to_sentences(table_to_dict(table), min_reliability=min_reliability)
...
def build_rag_chunks(
    ...
    """Build RAG indexable chunks from parsed YAML document (wrapper)."""
    for table in parsed.tables:
        if table.quality is None:
            table.quality = evaluate_table_quality(table)

    doc_dict = parsed_document_to_dict(parsed)
    return build_rag_chunks_from_dict(doc_dict, min_table_reliability=min_table_reliability)
...
class HWPParser:
    ...
    def _to_serializable_dict(self, doc: ParsedDocumentYAML) -> Dict[str, Any]:
        """Convert ParsedDocumentYAML to serializable dictionary."""
        return parsed_document_to_dict(doc)
